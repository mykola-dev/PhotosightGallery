...
abstract class JsoupRequest<T> : Request<T> {
    ...
    protected suspend fun getDocument(): Document =
            Ksoup.parse(
                    runHttpRequest(
                            url,
                            extraCookies +
...
class CategoriesRequest : JsoupRequest<List<PhotoCategory>>() {
    ...
    override suspend fun invoke(): List<PhotoCategory> =
            getDocument()
    ...
    override suspend fun invoke(): PhotoDetails {
        ...
        val comments =
                doc.commentsSection().map { comment ->
                    ...
                    val timestamp =
                            kotlinx.datetime.Instant.fromEpochMilliseconds(0) // TODO: fix date parsing
...
abstract class PhotosRequest : JsoupRequest<PhotosPage>() {
    ...
    override suspend fun invoke(): PhotosPage {
        val doc = getDocument()
        val photos =
...
class NewPhotosRequest(override val page: SimplePage) : PhotosRequest(), Multipage {
    override val url: String = "$baseUrl/photos/?pager=${page.key}"
}
